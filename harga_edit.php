<?php
    include "session.php";
    $id_barang_layanan = $_REQUEST['id_barang_layanan'];
    ...
    if(isset($_POST['simpan'])){
        ...
        while($row1 = $data->fetch_assoc()){
            if(($id_barang != $row1['id_barang']) && ($id_layanan != $row1['id_layanan'])){
                $stmt=$conn->prepare('UPDATE barang_layanan SET id_barang=?, id_layanan=?, harga=?, satuan=? WHERE id_barang_layanan=?');
                $stmt->bind_param("ssiss", $id_barang, $id_layanan, $harga, $satuan, $id_barang_layanan);
                $stmt->execute();
    ...
    $stmt = $conn->prepare("SELECT * FROM barang_layanan WHERE id_barang_layanan = ?");
    $stmt->bind_param("s", $id_barang_layanan);
    $stmt->execute();
    ...
    if(!$result){
        echo "<script>
                ...
                window.location='harga_data.php';
                </script>";
    }
                        ...
                        <div class="card-body">
                            <form class="row g-2 needs-validation" novalidate method="post">
                                <div class="col-md-12 form-group">
                                    <input type="hidden" class="form-control" style="background-color: #dee2e6" id="validationCustom01" pattern="^[A-Za-z]+([\A-Za-z]+)*" name="id_barang_layanan" value="<?php echo $id_barang_layanan?>" readonly required>
                                </div>
                                <div class="col-md-12 form-group">
                                    <label for="validationCustom4" class="form-label">Nama Barang</label>
                                    <select class="form-control" id="validationCustom04" name="id_barang" required>
                                        ...
                                        ?>
                                            ...
                                            <?php if($id_barang == $data['id_barang']) {
                                                echo 'selected';
                                            }?>>
                                ...
                                <div class="col-md-12 form-group">
                                    ...
                                    <select class="form-control" id="validationCustom04" name="id_layanan" required>
                                        <option selected disabled value="">Pilih Layanan</option>
                                        <?php
                                        $layanan = $conn->query("SELECT * FROM layanan");
                                        while($data = $layanan->fetch_assoc()){
                                        ?>
                                            <option value="<?php echo $data['id_layanan']; ?>"
                                            <?php if($id_layanan == $data['id_layanan']) {
                                                echo 'selected';
                                            }?>>
                                ...
                                <div class="col-md-12">
                                    ...
                                    <select class="form-control" style="background-color: #dee2e6" id="validationCustom04" name="satuan"  value="<?php echo $satuan?>"required>
                                        ...
                                        <?php 
                                            ...
                                            if($satuan =="Lembar") {
                                                echo"<option value='Lembar' selected>Lembar</option>";
                                            }else {
                                                echo "<option value='Lembar'>Lembar</option>";
                                            }
                                        ?>
                                    </select>
                                    <div class="invalid-feedback">
                                ...
                                <div class="col-md-12">
                                    ...
                                    <input autofocus type="number" min="1" class="form-control" style="background-color: #dee2e6" id="validationCustom08" pattern="[0-9]" name="harga" placeholder="Harga"  value="<?php echo $harga?>" required>
                                    <div class="invalid-feedback">
